...
var _LevelPlayBannerAdViewNativeComponent = _interopRequireWildcard(require("../specs/LevelPlayBannerAdViewNativeComponent"));
...
const LevelPlayBannerAdView = exports.LevelPlayBannerAdView = /*#__PURE__*/(0, _react.forwardRef)((props, ref) => {
  ...
  const loadAd = (0, _react.useCallback)(() => {
    bannerAdViewRef.current && _LevelPlayBannerAdViewNativeComponent.Commands.loadAd(bannerAdViewRef.current);
  }, []);

  // A method to destroy the banner ad
  const destroy = (0, _react.useCallback)(() => {
    bannerAdViewRef.current && _LevelPlayBannerAdViewNativeComponent.Commands.destroy(bannerAdViewRef.current);
  }, []);

  // A method to resume the auto refresh of the banner after it is paused
  const resumeAutoRefresh = (0, _react.useCallback)(() => {
    bannerAdViewRef.current && _LevelPlayBannerAdViewNativeComponent.Commands.resumeAutoRefresh(bannerAdViewRef.current);
  }, []);

  // A method to pause the auto refresh of loaded banner add
  const pauseAutoRefresh = (0, _react.useCallback)(() => {
    bannerAdViewRef.current && _LevelPlayBannerAdViewNativeComponent.Commands.pauseAutoRefresh(bannerAdViewRef.current);
  }, []);
  ...
  return /*#__PURE__*/_react.default.createElement(_LevelPlayBannerAdViewNativeComponent.default, _extends({
    ref: saveElement,
    creationParams: {
      adUnitId: adUnitId,
      adSize: adSize.toMap(),
